...
example_data = {'ER_before': 73.65,
                ...
                'P_NXD_USD_before': 1167.0,
                'TKXD_before': 0.0,
                'TKXD_after': 0.0,
                'TNXD_before': 0.0,
                'TNXD_after': 0.0,
                'SS_KPD_SUPPLY_after': 0.0,
                'SS_NPD_SUPPLY_after': 0.0,
                'SS_KXW_SUPPLY_after': 0.0,
                'SS_NXW_SUPPLY_after': 0.0,
                'DS_FCD_DEMAND_after': 0.0,
                'DS_FCW_DEMAND_after': 0.0,
                'KPD_pr': 41700.0,
                'KPD_q': 10670000.0,
                'KXD_q': 7220000.0,
                'NPD_pr': 59833.0,
...
class InputDataBase:
    def __init__(self, dict_from_frontend):
        ...
        self.SS_KPD_SUPPLY_after = float(dict_from_frontend.get('SS_KPD_SUPPLY_after'))
        self.SS_NPD_SUPPLY_after = float(dict_from_frontend.get('SS_NPD_SUPPLY_after'))
        self.SS_KXW_SUPPLY_after = float(dict_from_frontend.get('SS_KXW_SUPPLY_after'))
        self.SS_NXW_SUPPLY_after = float(dict_from_frontend.get('SS_NXW_SUPPLY_after'))
        self.DS_FCD_DEMAND_after = float(dict_from_frontend.get('DS_FCD_DEMAND_after'))
        self.DS_FCW_DEMAND_after = float(dict_from_frontend.get('DS_FCW_DEMAND_after'))
        ...
        self.Ω_KPD = float(dict_from_frontend.get('Ω_KPD'))
        self.Ω_NPD = float(dict_from_frontend.get('Ω_NPD'))
        self.σ_FCD = float(dict_from_frontend.get('σ_FCD'))
        self.σ_KSW = float(dict_from_frontend.get('σ_KSW'))
        self.σ_NSW = float(dict_from_frontend.get('σ_NSW'))
        self.σ_FCW = float(dict_from_frontend.get('σ_FCW'))
        self.ε_KPD = float(dict_from_frontend.get('ε_KPD'))
        self.ε_NPD = float(dict_from_frontend.get('ε_NPD'))
        self.ε_KXW = float(dict_from_frontend.get('ε_KXW'))
        self.ε_NXW = float(dict_from_frontend.get('ε_NXW'))
        self.ε_FCD = float(dict_from_frontend.get('ε_FCD'))
        self.ε_FCW = float(dict_from_frontend.get('ε_FCW'))
...
def fertilizers_market(input_data):
    ...
    SS_KPD_SUPPLY_1 = input_data.SS_KPD_SUPPLY_after
    SS_NPD_SUPPLY_1 = input_data.SS_NPD_SUPPLY_after
    SS_KXW_SUPPLY_1 = input_data.SS_KXW_SUPPLY_after
    SS_NXW_SUPPLY_1 = input_data.SS_NXW_SUPPLY_after
    DS_FCD_DEMAND_1 = input_data.DS_FCD_DEMAND_after
    DS_FCW_DEMAND_1 = input_data.DS_FCW_DEMAND_after
    ...
    Ω_KPD = input_data.Ω_KPD
    Ω_NPD = input_data.Ω_NPD
    σ_FCD = input_data.σ_FCD
    σ_KSW = input_data.σ_KSW
    σ_NSW = input_data.σ_NSW
    σ_FCW = input_data.σ_FCW
    ε_KPD = input_data.ε_KPD
    ε_NPD = input_data.ε_NPD
    ε_KXW = input_data.ε_KXW
    ε_NXW = input_data.ε_NXW
    ε_FCD = input_data.ε_FCD
    ε_FCW = input_data.ε_FCW
    ...
    TNXD_0 = TNXD_0 / 100
    TNXD_1 = TNXD_1 / 100
    SS_KPD_SUPPLY_1 = SS_KPD_SUPPLY_1 / 100
    SS_NPD_SUPPLY_1 = SS_NPD_SUPPLY_1 / 100
    SS_KXW_SUPPLY_1 = SS_KXW_SUPPLY_1 / 100
    SS_NXW_SUPPLY_1 = SS_NXW_SUPPLY_1 / 100
    DS_FCD_DEMAND_1 = DS_FCD_DEMAND_1 / 100
    DS_FCW_DEMAND_1 = DS_FCW_DEMAND_1 / 100
    ...
    Z_FCW = ((B_KSW * Q_KSW_0 ** r_σ_FCW + B_NSW * Q_NSW_0 ** r_σ_FCW) ** (1 / r_σ_FCW)) / \
            ((((B_KSW * (P_KSW_0 / B_KSW) ** (1 - σ_FCW) + B_NSW * (P_NSW_0 / B_NSW) ** (1 - σ_FCW)) ** (1 / (1 - σ_FCW))) / ER_0) ** ε_FCW)
    ...
    def func(z):
        ...
        KPD_SUPPLY_1 = np.abs(
            (stepen(K_KCD * stepen(Q_KCD_1, r_Ω_KPD) + K_KXD * stepen(Q_KXD_1, r_Ω_KPD), (1 / r_Ω_KPD)) - Z_KPD \
             * (1 + SS_KPD_SUPPLY_1) * stepen(stepen(K_KCD * stepen(P_KCD_1 / K_KCD, (1 + Ω_KPD)) + K_KXD *
                                                     stepen(P_KXD_1 / K_KXD, (1 + Ω_KPD)), (1 / (1 + Ω_KPD))), ε_KPD)))
        KPD_SUPPLY_2 = np.abs(P_KPD_1 - np.abs(P_KPD_1))
        KPD_SUPPLY = KPD_SUPPLY_1 + KPD_SUPPLY_2
        # print("{:20}".format('KPD_SUPPLY ='), "{:>32f}".format(KPD_SUPPLY))

        KPD_BUD_CET_1 = np.abs(P_KPD_1 * Q_KPD_1 - P_KXD_1 * Q_KXD_1 - P_KCD_1 * Q_KCD_1)
        KPD_BUD_CET_2 = np.abs(Q_KPD_1 - np.abs(Q_KPD_1))
        KPD_BUD_CET = KPD_BUD_CET_1 + KPD_BUD_CET_2
        # print("{:20}".format('KPD_BUD_CET ='), "{:>32f}".format(KPD_BUD_CET))

        KPD_CET_1 = np.abs(Q_KXD_1 / Q_KCD_1 - stepen((P_KXD_1 / P_KCD_1) * (K_KCD / K_KXD), (1 / (r_Ω_KPD - 1))))
        KPD_CET_2 = np.abs(P_KXD_1 - np.abs(P_KXD_1))
        KPD_CET = KPD_CET_1 + KPD_CET_2
        # print("{:20}".format('KPD_CET ='), "{:>32f}".format(KPD_CET))

        KPD_BAL_CET_1 = np.abs(Q_KPD_1 - Q_KCD_1 - Q_KXD_1)
        KPD_BAL_CET_2 = np.abs(Q_KXD_1 - np.abs(Q_KXD_1))
        KPD_BAL_CET = KPD_BAL_CET_1 + KPD_BAL_CET_2
        # print("{:20}".format('KPD_BAL_CET ='), "{:>32f}".format(KPD_BAL_CET))

        NPD_SUPPLY_1 = np.abs(
            stepen(K_NCD * stepen(Q_NCD_1, r_Ω_NPD) + K_NXD * stepen(Q_NXD_1, r_Ω_NPD), (1 / r_Ω_NPD)) - Z_NPD * \
            (1 + SS_NPD_SUPPLY_1) * stepen(stepen(K_NCD * stepen(P_NCD_1 / K_KCD, (1 + Ω_NPD)) + K_NXD *
                                                  stepen(P_NXD_1 / K_NXD, (1 + Ω_NPD)), (1 / (1 + Ω_NPD))), ε_NPD))
        NPD_SUPPLY_2 = np.abs(P_KCD_1 - np.abs(P_KCD_1))
        NPD_SUPPLY = NPD_SUPPLY_1 + NPD_SUPPLY_2
        # print("{:20}".format('NPD_SUPPLY ='), "{:>32f}".format(NPD_SUPPLY))

        NPD_BUD_CET_1 = np.abs(P_NPD_1 * Q_NPD_1 - P_NXD_1 * Q_NXD_1 - P_NCD_1 * Q_NCD_1)
        NPD_BUD_CET_2 = np.abs(Q_KCD_1 - np.abs(Q_KCD_1))
        NPD_BUD_CET = NPD_BUD_CET_1 + NPD_BUD_CET_2
        # print("{:20}".format('NPD_BUD_CET ='), "{:>32f}".format(NPD_BUD_CET))

        NPD_CET_1 = np.abs(Q_NXD_1 / Q_NCD_1 - stepen((P_NXD_1 / (P_NCD_1)) * (K_NCD / K_NXD), (1 / (r_Ω_NPD - 1))))
        NPD_CET_2 = np.abs(P_NPD_1 - np.abs(P_NPD_1))
        NPD_CET = NPD_CET_1 + NPD_CET_2
        # print("{:20}".format('NPD_CET ='), "{:>32f}".format(NPD_CET))

        FCD_BUD_CES_1 = np.abs(P_FCD_1 * Q_FCD_1 - P_KCD_1 * Q_KCD_1 - P_NCD_1 * Q_NCD_1)
        FCD_BUD_CES_2 = np.abs(Q_NPD_1 - np.abs(Q_NPD_1))
        FCD_BUD_CES = FCD_BUD_CES_1 + FCD_BUD_CES_2
        # print("{:20}".format('FCD_BUD_CES ='), "{:>32f}".format(FCD_BUD_CES))

        FCD_CES_1 = np.abs(Q_KCD_1 / Q_NCD_1 - stepen((P_KCD_1 / (P_NCD_1)) * (B_NCD / B_KCD), (1 / (r_σ_FCD - 1))))
        FCD_CES_2 = np.abs(P_NXD_1 - np.abs(P_NXD_1))
        FCD_CES = FCD_CES_1 + FCD_CES_2
        # print("{:20}".format('FCD_CES ='), "{:>32f}".format(FCD_CES))

        FCD_BAL_CES_1 = np.abs(Q_FCD_1 - Q_KCD_1 - Q_NCD_1)
        FCD_BAL_CES_2 = np.abs(Q_NXD_1 - np.abs(Q_NXD_1))
        FCD_BAL_CES = FCD_BAL_CES_1 + FCD_BAL_CES_2
        # print("{:20}".format('FCD_BAL_CES ='), "{:>32f}".format(FCD_BAL_CES))

        FCD_DEMAND_1 = np.abs(
            stepen(B_KCD * stepen(Q_KCD_1, r_σ_FCD) + B_NCD * stepen(Q_NCD_1, r_σ_FCD), (1 / r_σ_FCD)) - \
            Z_FCD * (1 + DS_FCD_DEMAND_1) * stepen(stepen(B_KCD * stepen(P_KCD_1 / B_KCD, (1 - σ_FCD)) +
                                                          B_NCD * stepen(P_NCD_1 / B_NCD, (1 - σ_FCD)),
                                                          (1 / (1 - σ_FCD))), ε_FCD))
        FCD_DEMAND_2 = np.abs(P_NCD_1 - np.abs(P_NCD_1))
        FCD_DEMAND = FCD_DEMAND_1 + FCD_DEMAND_2
        # print("{:20}".format('FCD_DEMAND ='), "{:>32f}".format(FCD_DEMAND))

        KSW_BUD_CES_1 = np.abs(P_KSW_1 * Q_KSW_1 - P_KXD_1 * (1 + TKXD_1) * Q_KXD_1 - P_KXW_1 * Q_KXW_1)
        KSW_BUD_CES_2 = np.abs(Q_NCD_1 - np.abs(Q_NCD_1))
        KSW_BUD_CES = KSW_BUD_CES_1 + KSW_BUD_CES_2
        # print("{:20}".format('KSW_BUD_CES ='), "{:>32f}".format(KSW_BUD_CES))

        KSW_CES_1 = np.abs(
            Q_KXW_1 / Q_KXD_1 - stepen((P_KXW_1 / (P_KXD_1 * (1 + TKXD_1))) * (B_KXD / B_KXW), (1 / (r_σ_KSW - 1))))
        KSW_CES_2 = np.abs(P_FCD_1 - np.abs(P_FCD_1))
        KSW_CES = KSW_CES_1 + KSW_CES_2
        # print("{:20}".format('KSW_CES ='), "{:>32f}".format(KSW_CES))

        KSW_BAL_CES_1 = np.abs(Q_KSW_1 - Q_KXD_1 - Q_KXW_1)
        KSW_BAL_CES_2 = np.abs(Q_FCD_1 - np.abs(Q_FCD_1))
        KSW_BAL_CES = KSW_BAL_CES_1 + KSW_BAL_CES_2
        # print("{:20}".format('KSW_BAL_CES ='), "{:>32f}".format(KSW_BAL_CES))

        KXW_SUPPLY_1 = np.abs(Q_KXW_1 - Z_KXW * (1 + SS_KXW_SUPPLY_1) * stepen(P_KXW_1 / ER_1, ε_KXW))
        KXW_SUPPLY_2 = np.abs(P_KXW_1 - np.abs(P_KXW_1))
        KXW_SUPPLY = KXW_SUPPLY_1 + KXW_SUPPLY_2
        # print("{:20}".format('KXW_SUPPLY ='), "{:>32f}".format(KXW_SUPPLY))

        NSW_BUD_CES_1 = np.abs(P_NSW_1 * Q_NSW_1 - P_NXD_1 * (1 + TNXD_1) * Q_NXD_1 - P_NXW_1 * Q_NXW_1)
        NSW_BUD_CES_2 = np.abs(Q_KXW_1 - np.abs(Q_KXW_1))
        NSW_BUD_CES = NSW_BUD_CES_1 + NSW_BUD_CES_2
        # print("{:20}".format('NSW_BUD_CES ='), "{:>32f}".format(NSW_BUD_CES))

        NSW_CES_1 = np.abs(
            Q_NXW_1 / Q_NXD_1 - stepen(((P_NXW_1) / (P_NXD_1 * (1 + TNXD_1))) * (B_NXD / B_NXW), (1 / (r_σ_NSW - 1))))
        NSW_CES_2 = np.abs(P_KSW_1 - np.abs(P_KSW_1))
        NSW_CES = NSW_CES_1 + NSW_CES_2
        # print("{:20}".format('NSW_CES ='), "{:>32f}".format(NSW_CES))

        NSW_BAL_CES_1 = np.abs(Q_NSW_1 - Q_NXD_1 - Q_NXW_1)
        NSW_BAL_CES_2 = np.abs(Q_KSW_1 - np.abs(Q_KSW_1))
        NSW_BAL_CES = NSW_BAL_CES_1 + NSW_BAL_CES_2
        # print("{:20}".format('NSW_BAL_CES ='), "{:>32f}".format(NSW_BAL_CES))

        NXW_SUPPLY_1 = np.abs(Q_NXW_1 - Z_NXW * (1 + SS_NXW_SUPPLY_1) * stepen(P_NXW_1 / ER_1, ε_NXW))
        NXW_SUPPLY_2 = np.abs(P_NXW_1 - np.abs(P_NXW_1))
        NXW_SUPPLY = NXW_SUPPLY_1 + NXW_SUPPLY_2
        # print("{:20}".format('NXW_SUPPLY ='), "{:>32f}".format(NXW_SUPPLY))

        FCW_BUD_CES_1 = np.abs(P_FCW_1 * Q_FCW_1 - P_KSW_1 * Q_KSW_1 - P_NSW_1 * Q_NSW_1)
        FCW_BUD_CES_2 = np.abs(Q_NXW_1 - np.abs(Q_NXW_1))
        FCW_BUD_CES = FCW_BUD_CES_1 + FCW_BUD_CES_2
        # print("{:20}".format('FCW_BUD_CES ='), "{:>32f}".format(FCW_BUD_CES))

        FCW_CES_1 = np.abs(Q_KSW_1 / Q_NSW_1 - stepen((P_KSW_1 / (P_NSW_1)) * (B_NSW / B_KSW), (1 / (r_σ_FCW - 1))))
        FCW_CES_2 = np.abs(P_NSW_1 - np.abs(P_NSW_1))
        FCW_CES = FCW_CES_1 + FCW_CES_2
        # print("{:20}".format('FCW_CES ='), "{:>32f}".format(FCW_CES))

        FCW_BAL_CES_1 = np.abs(Q_FCW_1 - Q_NSW_1 - Q_KSW_1)
        FCW_BAL_CES_2 = np.abs(Q_NSW_1 - np.abs(Q_NSW_1))
        FCW_BAL_CES = FCW_BAL_CES_1 + FCW_BAL_CES_2
        # print("{:20}".format('FCW_BAL_CES ='), "{:>32f}".format(FCW_BAL_CES))

        FCW_DEMAND_1 = np.abs(
            stepen(B_KSW * stepen(Q_KSW_1, r_σ_FCW) + B_NSW * stepen(Q_NSW_1, r_σ_FCW), (1 / r_σ_FCW)) - \
            Z_FCW * (1 + DS_FCW_DEMAND_1) * stepen(stepen(B_KSW * stepen(P_KSW_1 / B_KSW, (1 - σ_FCW))
                                                          + B_NSW * stepen(P_NSW_1 / B_NSW, (1 - σ_FCW)),
                                                          (1 / (1 - σ_FCW)) / ER_1), ε_FCW))
        FCW_DEMAND_2 = np.abs(P_FCW_1 - np.abs(P_FCW_1))
        FCW_DEMAND = FCW_DEMAND_1 + FCW_DEMAND_2
        # FCW_DEMAND=FCW_DEMAND_1
        # print("{:20}".format('FCW_DEMAND ='), "{:>32f}".format(FCW_DEMAND))

        TOT_BAL_1 = np.abs(Q_KPD_1 + Q_NPD_1 + Q_KXW_1 + Q_NXW_1 - Q_FCD_1 - Q_FCW_1)
        TOT_BAL_2 = np.abs(Q_FCW_1 - np.abs(Q_FCW_1))
        TOT_BAL = TOT_BAL_1 + TOT_BAL_2
        # print("{:20}".format('TOT_BAL ='), "{:>32f}".format(TOT_BAL))

        return [KPD_SUPPLY, KPD_BUD_CET, KPD_CET, KPD_BAL_CET, NPD_SUPPLY, NPD_BUD_CET, NPD_CET, FCD_BUD_CES, FCD_CES,
                FCD_BAL_CES, FCD_DEMAND, KSW_BUD_CES, KSW_CES, KSW_BAL_CES, KXW_SUPPLY, NSW_BUD_CES, NSW_CES,
                NSW_BAL_CES, NXW_SUPPLY, FCW_BUD_CES, FCW_CES, FCW_BAL_CES, FCW_DEMAND, TOT_BAL]
    ...
    while (eq_result >= 100) and (pass_cur < pass_num_max):
        ...
        Q_KCD_1 = solved_value[0]
        Q_KXD_1 = solved_value[1]
        P_KCD_1 = solved_value[2]
        P_KXD_1 = solved_value[3]
        P_KPD_1 = solved_value[4]
        Q_KPD_1 = solved_value[5]
        Q_NCD_1 = solved_value[6]
        Q_NXD_1 = solved_value[7]
        P_NCD_1 = solved_value[8]
        P_NXD_1 = solved_value[9]
        P_NPD_1 = solved_value[10]
        Q_NPD_1 = solved_value[11]
        P_FCD_1 = solved_value[12]
        Q_FCD_1 = solved_value[13]
        P_KSW_1 = solved_value[14]
        Q_KSW_1 = solved_value[15]
        P_KXW_1 = solved_value[16]
        Q_KXW_1 = solved_value[17]
        P_NSW_1 = solved_value[18]
        Q_NSW_1 = solved_value[19]
        P_NXW_1 = solved_value[20]
        Q_NXW_1 = solved_value[21]
        P_FCW_1 = solved_value[22]
        Q_FCW_1 = solved_value[23]
        ...
        KPD_SUPPLY_2 = np.abs(P_KPD_1 - np.abs(P_KPD_1))
        KPD_SUPPLY = KPD_SUPPLY_1 + KPD_SUPPLY_2
        eqs.append(KPD_SUPPLY)

        KPD_BUD_CET_1 = np.abs(P_KPD_1 * Q_KPD_1 - P_KXD_1 * Q_KXD_1 - P_KCD_1 * Q_KCD_1)
        KPD_BUD_CET_2 = np.abs(Q_KPD_1 - np.abs(Q_KPD_1))
        KPD_BUD_CET = KPD_BUD_CET_1 + KPD_BUD_CET_2
        eqs.append(KPD_BUD_CET)
        ...
        NPD_SUPPLY_2 = np.abs(P_KCD_1 - np.abs(P_KCD_1))
        NPD_SUPPLY = NPD_SUPPLY_1 + NPD_SUPPLY_2
        eqs.append(NPD_SUPPLY)

        NPD_BUD_CET_1 = np.abs(P_NPD_1 * Q_NPD_1 - P_NXD_1 * Q_NXD_1 - P_NCD_1 * Q_NCD_1)
        NPD_BUD_CET_2 = np.abs(Q_KCD_1 - np.abs(Q_KCD_1))
        NPD_BUD_CET = NPD_BUD_CET_1 + NPD_BUD_CET_2
        eqs.append(NPD_BUD_CET)
        ...
        FCD_DEMAND_2 = np.abs(P_NCD_1 - np.abs(P_NCD_1))
        FCD_DEMAND = FCD_DEMAND_1 + FCD_DEMAND_2
        eqs.append(FCD_DEMAND)

        KSW_BUD_CES_1 = np.abs(P_KSW_1 * Q_KSW_1 - P_KXD_1 * (1 + TKXD_1) * Q_KXD_1 - P_KXW_1 * Q_KXW_1)
        KSW_BUD_CES_2 = np.abs(Q_NCD_1 - np.abs(Q_NCD_1))
        KSW_BUD_CES = KSW_BUD_CES_1 + KSW_BUD_CES_2
        eqs.append(KSW_BUD_CES)
        ...
        KSW_BAL_CES_2 = np.abs(Q_FCD_1 - np.abs(Q_FCD_1))
        KSW_BAL_CES = KSW_BAL_CES_1 + KSW_BAL_CES_2
        eqs.append(KSW_BAL_CES)

        KXW_SUPPLY_1 = np.abs(Q_KXW_1 - Z_KXW * (1 + SS_KXW_SUPPLY_1) * stepen(P_KXW_1 / ER_1, ε_KXW))
        KXW_SUPPLY_2 = np.abs(P_KXW_1 - np.abs(P_KXW_1))
        KXW_SUPPLY = KXW_SUPPLY_1 + KXW_SUPPLY_2
        eqs.append(KXW_SUPPLY)

        NSW_BUD_CES_1 = np.abs(P_NSW_1 * Q_NSW_1 - P_NXD_1 * (1 + TNXD_1) * Q_NXD_1 - P_NXW_1 * Q_NXW_1)
        NSW_BUD_CES_2 = np.abs(Q_KXW_1 - np.abs(Q_KXW_1))
        NSW_BUD_CES = NSW_BUD_CES_1 + NSW_BUD_CES_2
        eqs.append(NSW_BUD_CES)
        ...
        NSW_BAL_CES_2 = np.abs(Q_KSW_1 - np.abs(Q_KSW_1))
        NSW_BAL_CES = NSW_BAL_CES_1 + NSW_BAL_CES_2
        eqs.append(NSW_BAL_CES)

        NXW_SUPPLY_1 = np.abs(Q_NXW_1 - Z_NXW * (1 + SS_NXW_SUPPLY_1) * stepen(P_NXW_1 / ER_1, ε_NXW))
        NXW_SUPPLY_2 = np.abs(P_NXW_1 - np.abs(P_NXW_1))
        NXW_SUPPLY = NXW_SUPPLY_1 + NXW_SUPPLY_2
        eqs.append(NXW_SUPPLY)

        FCW_BUD_CES_1 = np.abs(P_FCW_1 * Q_FCW_1 - P_KSW_1 * Q_KSW_1 - P_NSW_1 * Q_NSW_1)
        FCW_BUD_CES_2 = np.abs(Q_NXW_1 - np.abs(Q_NXW_1))
        FCW_BUD_CES = FCW_BUD_CES_1 + FCW_BUD_CES_2
        eqs.append(FCW_BUD_CES)
    ...
    if solution == False:
        ...
        Q_KXD_1 = 7220000
        P_KCD_1 = 8899.539999999983
        Q_KCD_1 = 3450000
        P_NPD_1 = 59833
        Q_NPD_1 = 11350000
        P_NXD_1 = 85949.55
        Q_NXD_1 = 6150000
        P_NCD_1 = 28945.157211538462
        Q_NCD_1 = 5200000
        P_FCD_1 = 20950.084450867045
        Q_FCD_1 = 8650000
        P_KXW_1 = 77666.2505204719
        Q_KXW_1 = 27379000
        P_KSW_1 = 73431.59802306426
        Q_KSW_1 = 34599000
        P_NXW_1 = 125061.02705917787
        Q_NXW_1 = 33205000
