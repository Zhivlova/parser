...
user_data = {'ER_before': 75.0, 'ER_after': 80.0, 'WP_before': 2000.0, 'WP_after': 2088.0,
                'P_AXD_USD_before': 1842.0, 'P_AMD_USD_before': 2377.0, 'P_IMD_USD_before': 353.0, 'TAXD_before': 50.0,
                'TAXD_after': 0.3, 'TIMD_before': 0.0, 'TIMD_after': 0.0, 'TAMD_before': 0.0, 'TAMD_after': 0.0,
                'SS_IPD_SUPPLY_after': 0.0, 'DS_IMD_SUPPLY_after': 0.0, 'SS_AVD_SUPPLY_after': 0.0,
                'DS_ADW_DEMAND_after': 0.0, 'SS_AXW_SUPPLY_after': 0.0, 'SS_AMD_SUPPLY_after': 0.0,
                'DS_ATD_DEMAND_after': 0.0, 'IPD_pr': 32821.0, 'IPD_q': 8200000.0, 'IMD_q': 4722500.0,
...
def aluminum_market(input_data):
    ...
    def func(z):
        ...
        IPD_SUPPLY = Q_IPD_1 - parameters.at[6, 'rho'] * (
                1 + model_control_actions.at[12, 'after']) * (P_IPD_1) ** parameters.at[6, 'values']
        ...
        AID_BUD_CES = P_AID_1*Q_AID_1-P_IMD_1*(1+model_control_actions.at[9, 'after'])*Q_IMD_1-P_IPD_1*Q_IPD_1
        ...
        AVD_SUPPLY = Q_AVD_1-parameters.at[8, 'rho']*(1+model_control_actions.at[14, 'after'])*\
                     (P_AVD_1) ** parameters.at[8, 'values']

        APD_BUD_CES = P_APD_1*Q_APD_1-P_AID_1*Q_AID_1-P_AVD_1*Q_AVD_1

        APD_CES = Q_AVD_1/Q_AID_1-((P_AVD_1/(P_AID_1))*(variables.at[25, 'relative_quality'] /
                            variables.at[26, 'relative_quality']))**(1/(parameters.at[1, 'rho']-1))

        APD_BAL_CES = Q_APD_1-variables.at[27, 'relative_quality']*(variables.at[25, 'relative_quality']*
                    Q_AID_1 ** parameters.at[1, 'rho']+variables.at[26, 'relative_quality']*
                                                    Q_AVD_1 ** parameters.at[1, 'rho']) ** (1/parameters.at[1, 'rho'])

        APD_BUD_CET = P_APD_1*Q_APD_1-P_AXD_1*Q_AXD_1-P_ASD_1*Q_ASD_1

        APD_CET = Q_AXD_1/Q_ASD_1-((P_AXD_1/(P_ASD_1))*(variables.at[31, 'relative_quality'] /
                        variables.at[28, 'relative_quality'])) ** (1/(parameters.at[2, 'rho']-1))

        APD_BAL_CET = Q_APD_1-Q_AXD_1-Q_ASD_1

        ADW_BUD_CES = P_ADW_1*Q_ADW_1-P_AXD_1*(1+model_control_actions.at[8, 'after'])*Q_AXD_1-P_AXW_1*Q_AXW_1
        ...
        ATD_BUD_CES = P_ATD_1*Q_ATD_1-P_AMD_1*(1+model_control_actions.at[10, 'after'])*Q_AMD_1-P_ASD_1*Q_ASD_1
        ...
        ADW_DEMAND_2 = Q_ATD_1-parameters.at[12, 'rho']*(1+model_control_actions.at[18, 'after'])*(P_ATD_1) ** parameters.at[12, 'values']
    ...
    z0 = [variables.at[23, 'base_quan'], variables.at[23, 'base_pr'], variables.at[24, 'base_quan'],
          variables.at[24, 'base_pr'], variables.at[25, 'base_pr'], variables.at[25, 'base_quan'],
